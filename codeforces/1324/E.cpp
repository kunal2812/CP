...
*/
int recur(int i, int curr, int h, int l, int r, vi &a, vector<vi> &dp){
    if(i==sz(a)){
        return 0;
    }
    else if(dp[i][curr]!=-1){
        ...
        int t1 = (a[i]+curr)%h;
        int t2 = (a[i]-1+curr)%h;
        return dp[i][curr]=max((t1>=l && t1<=r)+recur(i+1,t1,h,l,r,a,dp), (t2>=l && t2<=r)+recur(i+1,t2,h,l,r,a,dp));
    }
}
void solve(int xx){ 
    int n,h,l,r; cin >> n >> h >> l >> r;
    vi a(n); scna(a);
    vector<vi> dp(n,vi(h,-1));
    cout << recur(0,0,h,l,r,a,dp) << endl;
}
