...
void solve(int xx){ 
    ...
    int l = 1, r = n;
    while(l<=r){
        // cout << l << " " << r << " " << ms.begin()->ff << " " << ms.rbegin()->ff << endl;
        if((l!=ms.begin()->sc && r!=ms.rbegin()->sc) && (r!=ms.begin()->sc && l!=ms.rbegin()->sc)){
            cout << l << " " << r << endl; return;
        }
        else{
            if(l==ms.begin()->sc || l==ms.rbegin()->sc){
                // cout << "Erase: " << a[l] << " " << l << endl;
                ms.erase({a[l-1],l});
                l++;
            }
            else{
