...
ll recur(bool flag, int sum, int k, int d, vector<vll> &dp){
    if(sum<0){
        return 0;
    }
    else if(sum==0){
        ...
        fr(j,1,k+1,1){
            if(j>=d || flag){
                ans+=recur(true,sum-j,k,d,dp);ans%=mod;
            }
            else{
                ans+=recur(false,sum-j,k,d,dp); ans%=mod;
            }
        }
        return dp[flag][sum] = ans;
    }
}

void solve(int xx){ 
    int n,k,d; cin >> n >> k>> d;
    vector<vll> dp(2,vll(n+1,-1));
    pr(recur(false,n,k,d,dp));
}
