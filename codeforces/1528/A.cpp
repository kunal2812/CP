...
void dfs(int x, int p){
    for(auto u:tr[x]){
        ...
        dfs(u,x);
        dp[x][1]+=max(dp[u][1]+abs(val[u].sc-val[x].sc),dp[u][0]+abs(val[u].ff-val[x].sc));
        dp[x][0]+=max(dp[u][1]+abs(val[u].sc-val[x].ff),dp[u][0]+abs(val[u].ff-val[x].ff));
    }
}
void solve(int xx){ 
    int n; cin >> n;
    fr(i,1,n+1,1){
        tr[i].clear(); dp[i][0] = dp[i][1] = 0;
        cin >> val[i].ff >> val[i].sc;
    }
    fr(i,1,n,1){
        int u,v; cin >> u >> v;
        tr[u].pb(v); tr[v].pb(u);
